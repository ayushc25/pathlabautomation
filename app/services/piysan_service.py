...
class PiysanClient:
    ...
    def get_bookings(self, token: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        logger.info("Fetching bookings from Piysan LIS at %s (limit=%s, offset=%s)", url, limit, offset)
        with httpx.Client(timeout=15.0) as client:
            ...
            if isinstance(data, list):
                ...
                # Standard wrapper unwrapping
                bookings = data.get("bookings") or data.get("data") or data.get("results")
                if isinstance(bookings, list):
                    return bookings
                return [data]
            return []

    def submit_report(self, token: str, reference_no: str, results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    with _lock, _connect(MAPPING_DB_PATH) as conn:
        for b in bookings:
            ...
            if isinstance(patient_info, dict):
                ...
                # The Piysan LIS API does not return a patient ID field - fall back to mobile as the identifier.
                patient_id = (
                    patient_info.get("id")
            ...
            conn.execute(
                ...
                """,
                (ref_no, patient_name, patient_id, json.dumps(b, ensure_ascii=False)),
            )

            # Auto-import tests to lab_test_master so they populate the mappings list suggestions
            machines = b.get("machines") or []
            for m in machines:
...
def load_piysan_bookings() -> List[Dict[str, Any]]:
    """Load all locally cached bookings."""
    init_piysan_db()
    with _lock, _connect(MAPPING_DB_PATH) as conn:
        rows = conn.execute("SELECT * FROM piysan_bookings ORDER BY fetched_at DESC").fetchall()
        bookings = []
        for r in rows:
            b_dict = dict(r)
            try:
...
def find_latest_decoded_capture(sample_id: str) -> Optional[Dict[str, Any]]:
    ...
    with _lock, _connect(DECODED_DB_PATH) as conn:
        rows = conn.execute("SELECT raw_capture_id, payload FROM decoded_captures ORDER BY id DESC").fetchall()
        for r in rows:
            try:
